...
import { X, FileText, Download, Eye, CheckCircle2, CalendarDays, Copy, AlertTriangle, CircleCheck, CircleX, ChevronDown, Sparkles } from "lucide-react"
...
interface AllErrorsDetailPanelProps {
  open: boolean
  onClose: () => void
  row: AllErrorRow | null
}
  ...
  return (
    <Tooltip delayDuration={200}>
      <TooltipTrigger asChild>
        <p className="text-xs text-muted-foreground font-mono leading-relaxed break-all line-clamp-3 cursor-default">
          {content}
        </p>
      </TooltipTrigger>
      <TooltipContent
        side="top"
        className="max-w-md bg-foreground text-background text-xs font-mono p-3 leading-relaxed break-all whitespace-pre-wrap"
      >
        {content}
      </TooltipContent>
    </Tooltip>
...
export function AllErrorsDetailPanel({ open, onClose, row }: AllErrorsDetailPanelProps) {
          ...
          <div className="p-6">
            ...
            <div className="rounded-lg bg-accent p-4 mb-6">
              <div className="flex items-center justify-between mb-3">
                ...
                <Button variant="outline" size="sm" className="bg-background" onClick={() => setEdiModalOpen(true)}>
                  View File
                </Button>
              </div>
              <p className="text-sm font-medium text-foreground mb-2">{row.errorTitle}</p>
